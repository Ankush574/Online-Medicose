...
const DoctorDashboard = () => {
  ...
  return (
    <main className="doctor-dashboard-main" role="main" aria-label="Doctor Dashboard Main Content">
      ...
      <section aria-labelledby="patients-list">
        <h2 id="patients-list" tabIndex={0}>Patients List</h2>
        <ul>
          {patients.map((patient, idx) => (
            <li key={idx} tabIndex={0}>
              <span>{patient.name}</span>
              <span>{patient.status}</span>
              <button aria-label={`View details for ${patient.name}`}>Details</button>
            </li>
          ))}
        </ul>
      </section>
