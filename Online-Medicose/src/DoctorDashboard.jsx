...
const DoctorDashboard = () => {
  ...
  // Load clinical refills + prescriptions from backend for doctor views
  useEffect(() => {
    const loadClinicalData = async () => {
      if (!user?.token) return;
      try {
        const headers = { Authorization: `Bearer ${user.token}` };
        const [refillsRes, prescriptionsRes] = await Promise.all([
          fetch(`${API_BASE_URL}/clinical/refills`, { headers }),
          fetch(`${API_BASE_URL}/clinical/prescriptions`, { headers }),
        ]);

        if (refillsRes.ok) {
          const data = await refillsRes.json();
          if (Array.isArray(data)) {
            const mappedRefills = data.map((refill) => ({
              id: refill._id || refill.id,
              patientName:
                refill.userEmail?.split("@")[0] || "Patient",
              medication: refill.medicineName || "Medicine",
              requestedOn: new Date(
                refill.requestedAt || refill.createdAt || Date.now()
            ...
            setRefillApprovals((prev) => [...mappedRefills, ...prev]);
          }
        }
        ...
        if (prescriptionsRes.ok) {
          ...
          if (Array.isArray(data)) {
            const mappedRx = data.map((rx) => ({
              id: rx._id || rx.id,
              patientName:
                rx.userEmail?.split("@")[0] || "Patient",
              medication: rx.medicineName || "Medicine",
              dosage: rx.dosage || rx.strength || "",
              route: "Oral",
              duration: rx.duration || "--",
              refills: typeof rx.refillCount === "number" ? rx.refillCount : 0,
              status: rx.status || "Active",
              diagnosis: rx.notes ? "See notes" : "--",
              controlled: false,
              lastAction: new Date(
                rx.updatedAt || rx.createdAt || Date.now()
              ).toLocaleDateString("en-US", {
                month: "short",
                day: "2-digit",
              }),
  ...
  const handleRefillDecision = (requestId, approve) => {
    ...
    if (user?.token && requestId) {
      const headers = {
        "Content-Type": "application/json",
        Authorization: `Bearer ${user.token}`,
      };
      fetch(`${API_BASE_URL}/refills/${requestId}`, {
        method: "PATCH",
        headers,
