...
const PharmacistDashboard = () => {
  ...
  // Load backend refills and prescriptions for pharmacist views
  useEffect(() => {
      ...
      try {
        ...
        if (refillsRes.ok) {
          ...
          if (Array.isArray(data)) {
            ...
            setRefillRequests((prev) => [...mappedRefills, ...prev]);
          }
        }
        ...
        if (prescriptionsRes.ok) {
          ...
          if (Array.isArray(data)) {
            const mappedRx = data.map((rx) => ({
              ...
              date: (rx.createdAt || new Date().toISOString()).slice(0, 10),
            }));
            setPrescriptions((prev) => [...mappedRx, ...prev]);
          }
        }
      } catch (err) {
        console.error("Failed to load pharmacist clinical data", err);
      }
    };
  ...
  const handlePrescriptionAction = (id, action) => {
    ...
    if (user?.token && id) {
      ...
      const status = action === "fill" ? "Dispensed" : "Rejected";
