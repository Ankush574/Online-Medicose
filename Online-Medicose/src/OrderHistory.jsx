...
const OrderHistory = ({ embedded = false }) => {
			...
			try {
				...
				if (Array.isArray(data) && data.length > 0) {
					const mapped = data.map((order) => ({
						id: order.id || order._id || "ORD-" + (order.createdAt || "").slice(0, 10),
						placedOn: new Date(order.createdAt || Date.now()).toLocaleDateString("en-US", {
							month: "short",
							day: "2-digit",
							year: "numeric",
						}),
						...
						eta: "Pending update",
						deliveryWindow: "To be scheduled",
						total: order.totalAmount || 0,
						pharmacy: "Online MediCose Pharmacy",
						trackingId: order.trackingId || "N/A",
						channel: "Home delivery",
						items: (order.items || []).map((item, index) => ({
							id: item.id || item.medicine || `item-${index}`,
							name: item.name || "Medicine",
							quantity: item.quantity || 1,
							price: item.price || 0,
						})),
						shippingAddress: order.shippingAddress || "Saved address",
						paymentMethod: order.paymentMethod || "Online payment",
						coolChain: order.coolChain || "Standard",
						timeline: order.timeline || [
							{ id: "placed", label: "Order placed", timestamp: "Recently", status: "done" },
							{ id: "processing", label: "Processing", timestamp: "--", status: "current" },
						],
					}));
					setOrders(mapped);
				} else {
				...
				setError("Unable to load orders from server. Showing sample history.");
				setOrders(orderSeed);
			} finally {
				setLoading(false);
			}
	...
	const workspace = (
		<section className="orders-shell">
			...
			{loading && <p className="muted" style={{ marginTop: "0.25rem" }}>Loading latest orders…</p>}
