...
const VideoConsultation = ({ embedded = false }) => {
  ...
  useEffect(() => {
    // When a consultation is selected, try to start camera
    if (!selectedConsultation || !isVideoOn) {
      stopMediaStream();
      return;
    }
  ...
  }, [selectedConsultation, isVideoOn]);
                ...
                {cameraError ? (
                  ...
                  <>
                    <video
                      ...
                      muted
                    />
                    <div className="placeholder-video overlay-info">
                      <span className="doctor-avatar">{selectedConsultation.doctorImage}</span>
                      <h3>{selectedConsultation.doctorName}</h3>
                      <p>{selectedConsultation.specialization}</p>
                    </div>
                    <div className="remote-video-sim">
                      <div className="remote-video-inner">
                        <span className="remote-label">Doctor</span>
                        <span className="doctor-avatar-small">{selectedConsultation.doctorImage}</span>
                      </div>
                    </div>
                  </>
                )}
                ...
                {upcomingConsultations.map(consultation => (
                  <div key={consultation.id} className="consultation-card">
                    <div className="consultation-header">
                      <span className="doctor-icon">{consultation.doctorImage}</span>
          ...
          <section className="how-it-works">
            ...
            <div className="steps-grid">
              ...
              <div className="step-card">
                ...
                <h3>Consult</h3>
                <p>Talk to your doctor via secure video link.</p>
              </div>
              <div className="step-card">
  ...
  return (
    <section className="video-consultation-main" role="main" aria-label="Video Consultation Main Content">
      <header className="consultation-hero">
        <div>
          <p className="eyebrow">Virtual Care Studio</p>
          <h1 id="video-consultation-heading" tabIndex={0}>Video Consultation</h1>
          <p className="subcopy">Launch secure HD calls, capture notes, and trigger reminders without leaving the dashboard.</p>
        </div>
        <div className="hero-actions">
          <button className="ghost-btn" onClick={() => alert("Room link copied")}>Copy room link</button>
          <Link to="/dashboard" className="link-pill">Dashboard Home</Link>
        </div>
      </header>
