...
const MedicationRefill = ({ embedded = false }) => {
  ...
  // Load refill requests from backend when available
  useEffect(() => {
    const loadRefills = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/refills?${params.toString()}` , {
          headers,
        });
        if (!response.ok) return;
        const data = await response.json();
        if (!Array.isArray(data) || data.length === 0) return;
        const mapped = data.map((refill) => {
          const backendStatus = (refill.status || "Pending").toLowerCase();
          let status = "pending";
          if (backendStatus === "approved") status = "approved";
          else if (backendStatus === "rejected" || backendStatus === "cancelled") status = "cancelled";
          else if (backendStatus === "processing") status = "processing";
          return {
            id: refill._id || refill.id,
            medication: refill.medicineName || "Medicine",
            quantity: refill.quantity || 1,
            pharmacy: "Online MediCose Pharmacy",
            status,
            requestedOn: new Date(refill.requestedAt || refill.createdAt || Date.now()).toLocaleDateString("en-US", {
              month: "short",
              day: "2-digit",
              year: "numeric",
            }),
            requestedAt: Date.parse(refill.requestedAt || refill.createdAt || new Date().toISOString()),
            eta: "Pending confirmation",
            etaWindow: "To be scheduled",
            channel: "Home delivery",
            cost: "--",
            trackingId: refill.trackingId || "N/A",
            etaDays: 2,
            nextStep: status === "pending" ? "Awaiting prescriber approval" : "In pharmacy queue",
          };
        });
